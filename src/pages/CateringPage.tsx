  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    supabase.functions.invoke("print-request", {
      body: {
        ...
        details: [
          "ART: CATERING ANFRAGE",
          selectedPackage ? `PAKET: ${selectedPackage.name}` : "",
          `PERSONEN: ${persons}`,
          form.company ? `FIRMA: ${form.company}` : "",
          form.date ? `DATUM: ${form.date}` : "",
          form.time ? `UHRZEIT: ${form.time}` : "",
          [form.street, [form.plz, form.city].filter(Boolean).join(" ")].filter(Boolean).join(", ")
            ? `LIEFERADRESSE: ${[form.street, [form.plz, form.city].filter(Boolean).join(" ")].filter(Boolean).join(", ")}`
            : "",
